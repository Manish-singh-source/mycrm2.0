...
const departmentSchema = z.object({ name: z.string().trim().min(2), parent_uuid: z.string().optional(), manager_platform_user_uuid: z.string().optional(), status: z.enum(['active', 'inactive']) });
const designationSchema = z.object({ name: z.string().trim().min(2), description: z.string().optional(), level: z.coerce.number().int().min(0).optional(), status: z.enum(['active', 'inactive']) });
...
function DepartmentForm({ record }: { record?: OrganizationRecord }) {
  ...
  const form = useForm<DepartmentForm>({ resolver: zodResolver(departmentSchema), defaultValues: { name: textOf(record, ['name'], ''), parent_uuid: String(record?.parent?.uuid ?? ''), manager_platform_user_uuid: String(record?.manager?.uuid ?? ''), status: textOf(record, ['status'], 'active') as 'active' | 'inactive' } });
...
function DesignationForm({ record }: { record?: OrganizationRecord }) {
  ...
  const form = useForm<DesignationForm>({ resolver: zodResolver(designationSchema), defaultValues: { name: textOf(record, ['name'], ''), description: textOf(record, ['description'], ''), level: record?.level ?? undefined, status: textOf(record, ['status'], 'active') as 'active' | 'inactive' } });
  const mutation = useMutation({ mutationFn: (values: DesignationForm) => { const payload: DesignationPayload = { ...values, level: values.level ?? null }; return record ? platformOrganizationApi.designations.update(idOf(record), payload) : platformOrganizationApi.designations.create(payload); }, onSuccess: async saved => { await queryClient.invalidateQueries({ queryKey: platformQueryKeys.resource('platform-designations') }); navigate(`${definition.path}/${idOf(saved)}`); } });
  return <OrganizationFormShell title={record ? `Edit ${textOf(record, ['name'])}` : 'Create Designation'} backTo={definition.path} permission={record ? 'platform_designation.edit' : 'platform_designation.create'} error={mutation.error} saving={mutation.isPending} onSubmit={form.handleSubmit(values => mutation.mutate(values))}><FormField form={form} name="name" label="Designation name" required /><FormField form={form} name="level" label="Level" type="number" /><FormField form={form} name="description" label="Description" type="textarea" /><SelectField form={form} name="status" label="Status" options={[{ value: 'active', label: 'Active' }, { value: 'inactive', label: 'Inactive' }]} /></OrganizationFormShell>;
}
...
function FormField({ form, name, label, required = false, type = 'text', readOnly = false }: { form: any; name: string; label: string; required?: boolean; type?: string; readOnly?: boolean }) { const error = form.formState.errors[name]?.message; return <label className={error ? 'form-field-invalid' : undefined}><span>{label}{required ? ' *' : ''}</span>{type === 'textarea' ? <textarea {...form.register(name)} /> : <input type={type} readOnly={readOnly} {...form.register(name)} />}{error ? <strong role="alert">{String(error)}</strong> : null}</label>; }
