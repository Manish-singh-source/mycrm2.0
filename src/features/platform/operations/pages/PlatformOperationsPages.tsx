...
import {
  ...
  type PlatformRecord
} from '@/features/platform/operations/api/platformOperationsApi';
import { platformSubscriptionsApi } from '@/features/platform/subscriptions/api/platformSubscriptionsApi';
...
export function PlatformModulesPage() {
  return <PlatformModulesListPage />;
}
export function PlatformModuleCreatePage() {
  return <PlatformModuleFormPage mode="create" />;
}
export function PlatformModuleEditPage() {
  return <PlatformModuleFormPage mode="edit" />;
}
export function PlatformModuleViewPage() {
  return <PlatformModuleDetailPage />;
}
export function PlatformSupportTicketsPage() {
  return <OperationsPage config={configs['support-tickets']} />;
}
export function PlatformSupportTicketViewPage() {
  ...
  const detail = Object.fromEntries(
    Object.entries(mask(ticket) as Record<string, unknown>).filter(
      ([key]) => !['comments', 'attachments', 'audit'].includes(key)
    )
  );
    ...
    <section className="enterprise-module-page">
      <PageHeader
        ...
        description={printable(
          ticket.subject ?? 'Ticket detail, replies, files, and audit history.'
        )}
        actions={
          <Button
            type="button"
...
export function PlatformKnowledgeBasePage() {
  return <OperationsPage config={configs['knowledge-base']} />;
}
export function PlatformRemoteLoginPage() {
  return <OperationsPage config={configs['remote-login']} />;
}
export function PlatformReportsPage() {
  return <OperationsPage config={configs.reports} />;
}
export function PlatformMonitoringPage() {
  return <OperationsPage config={configs.monitoring} />;
}
export function PlatformIntegrationsPage() {
  return <OperationsPage config={configs.integrations} />;
}
export function PlatformSettingsPage() {
  return <OperationsPage config={configs.settings} />;
}
export function PlatformAuditPage() {
  return <OperationsPage config={configs.audit} />;
}
export function PlatformOnboardingPage() {
  return <OperationsPage config={configs.onboarding} />;
}
export function PlatformTrialsPage() {
  return <OperationsPage config={configs.trials} />;
}
export function PlatformLegalPage() {
  return <OperationsPage config={configs.legal} />;
}
export function PlatformAnnouncementsPage() {
  return <OperationsPage config={configs.announcements} />;
}
export function PlatformWebhooksPage() {
  return <OperationsPage config={configs.webhooks} />;
}

type ModuleModal =
  'filters' | 'views' | 'columns' | 'features' | 'delete' | 'bulkDelete' | 'toggle' | null;
...
function PlatformModulesListPage() {
  ...
  const mutation = useMutation({
    mutationFn: async ({
      ...
      action: 'delete' | 'bulkDelete' | 'toggle';
      ...
      if (action === 'toggle')
        return target.status === 'active'
          ? platformOperationsApi.modules.disable(id)
          : platformOperationsApi.modules.enable(id);
  ...
  return (
    <section className="enterprise-module-page platform-operations-page">
      <PageHeader
        ...
        actions={
          <PermissionButton
            ...
            type="button"
            onClick={() => navigate(`${moduleRoute}/create`)}
          >
      ...
      <ModuleStats
        rows={rows}
        stats={query.data?.meta?.stats as ModuleStatsData | undefined}
      ...
      <ModuleFeatureModal
        ...
        onComplete={() => {
          setModal(null);
          void queryClient.invalidateQueries({ queryKey: platformQueryKeys.resource('modules') });
        }}
      />
      <ConfirmDialog
        open={modal === 'toggle'}
...
function PlatformModuleFormPage({ mode }: { mode: 'create' | 'edit' }) {
  ...
  const mutation = useMutation({
    mutationFn: () =>
      mode === 'create'
        ? platformOperationsApi.modules.create(cleanModulePayload(form))
        : platformOperationsApi.modules.update(id, cleanModulePayload(form)),
  ...
  return (
    <section className="enterprise-module-page platform-operations-page">
      <PageHeader
        ...
        actions={
          <Button type="button" variant="secondary" onClick={() => navigate(moduleRoute)}>
            Back
          </Button>
        }
      />
      {detailQuery.isError ? (
        <div className="surface-error">{errorMessage(detailQuery.error)}</div>
      ) : null}
        ...
        actions={
          <>
            ...
            <Button
              type="button"
              variant="secondary"
              onClick={() => navigate(`${moduleRoute}/${id}/edit`)}
            >
...
function ModuleDetailTabs({
  module,
  active
}: {
  module: PlatformRecord;
  active: 'overview' | 'features' | 'activity';
}) {
  if (active === 'features')
    return <RelatedRows title="Features" rows={arrayFromResponse(module.features, 'features')} />;
  if (active === 'activity')
    return <RelatedRows title="Activity" rows={arrayFromResponse(module.activity, 'activity')} />;
  const detail = Object.fromEntries(
    Object.entries(module).filter(
      ([key]) => !['features', 'tenant_overrides', 'activity'].includes(key)
    )
  );
...
function ModuleStats({
  ...
  const activeRecords = Number(
    stats?.active ?? rows.filter((row) => row.status === 'active').length
  );
  return (
  ...
  return (
    <section className="platform-access-summary">
      ...
      <SummaryTile
        icon={<CheckCircle2 />}
        label="Active"
        value={module.status === 'active' ? '1' : '0'}
      />
    </section>
  );
}
  ...
  const allColumns: DataTableColumn<PlatformRecord>[] = [
    {
      id: 'name',
      header: 'Name',
      accessor: (row) => printable(row.name),
      enableSorting: true,
      cell: (row) => <strong>{printable(row.name)}</strong>
    },
    {
      id: 'code',
      header: 'Code',
      accessor: (row) => printable(row.code),
      enableSorting: true,
      cell: (row) => printable(row.code)
    },
    {
      id: 'category',
      header: 'Category',
      accessor: (row) => printable(row.category),
      cell: (row) => printable(row.category)
    },
    {
      id: 'icon',
      header: 'Icon',
      accessor: (row) => printable(row.icon),
      cell: (row) => printable(row.icon)
    },
    {
      id: 'is_core',
      header: 'Core',
      accessor: (row) => Boolean(row.is_core),
      cell: (row) => printable(row.is_core)
    },
    {
      id: 'status',
      header: 'Status',
      accessor: (row) => printable(row.status),
      cell: (row) => (
        <StatusBadge tone={tone(String(row.status ?? 'neutral'))}>
          {printable(row.status)}
        </StatusBadge>
      )
    },
    {
      id: 'sort_order',
      header: 'Sort Order',
      accessor: (row) => Number(row.sort_order ?? 0),
      cell: (row) => printable(row.sort_order ?? 0)
    },
    {
      id: 'actions',
      header: 'Actions',
  ...
  return (
    <div className="action-dropdown">
      <button
        ...
        aria-label={`Open actions for ${printable(row.name ?? row.code)}`}
        aria-expanded={open}
      ...
      <PortalActionMenu open={open} anchorRef={triggerRef} onClose={() => setOpen(false)}>
        <div className="action-menu" role="menu">
          ...
          <button
            ...
            role="menuitem"
            onMouseDown={(event) => event.preventDefault()}
            onClick={() => run(() => onFeatures(row))}
          >
            <Wrench size={15} aria-hidden /> Add Feature
          </button>
          <hr />
          <button
            type="button"
...
function ModuleTableModals({
  ...
  return (
    <>
      <AppModal open={modal === 'views'} onClose={onClose} title="Saved views">
        <div className="form-grid">
          <label className="check-row">
            <input type="radio" checked readOnly /> Default table
          </label>
        </div>
      </AppModal>
      <AppModal open={modal === 'filters'} onClose={onClose} title="Filters">
        <div className="form-grid">
          <label>
      ...
      <AppModal open={modal === 'columns'} onClose={onClose} title="Columns">
        <div className="form-grid">
          {columns
  ...
  return (
    <AppModal
      ...
      footer={
        <>
          ...
          <Button
            type="button"
            disabled={!moduleId || optionsQuery.isLoading}
    ...
    >
      ...
      {optionsQuery.isError ? (
        <div className="surface-error">{errorMessage(optionsQuery.error)}</div>
      ) : null}
      <CheckboxSelect
...
function recordFromResponse(payload: unknown, key: string): PlatformRecord {
  ...
  return record[key] && typeof record[key] === 'object' && !Array.isArray(record[key])
    ? (record[key] as PlatformRecord)
...
function OperationsPage({ config }: { config: AreaConfig }) {
  ...
  const initialTab =
    requestedTab && config.tabs.some((entry) => entry.id === requestedTab)
      ? requestedTab
      : config.tabs[0].id;
  ...
  const columns = useMemo(
    () => buildColumns(tab.columns, tab.primary ?? [], tab.actions ?? [], handleAction, activeTab),
    [activeTab, handleAction, tab]
  );
      ...
      <PageHeader
        ...
        actions={
          <HeaderActions
            config={config}
            activeTab={activeTab}
            onAction={(key, record) => setAction({ key, record, tab: activeTab })}
          />
        }
      />
      <Tabs
      ...
      <DataTable
        ...
        emptyState={
          <div className="empty-state">
            {tab.empty ??
              'No records returned. The route and module shell are ready for implementation.'}
          </div>
        }
        searchValue={search}
        ...
        onSelectionChange={setSelectedIds}
        onOpenExport={() =>
          setAction({ key: config.id === 'audit' ? 'auditExport' : 'reportExport', tab: activeTab })
        }
        page={page}
        total={query.data?.total ?? rows.length}
        onPageChange={setPage}
...
function HeaderActions({
  ...
  config: AreaConfig;
  activeTab: string;
  onAction: (key: ActionKey, record?: PlatformRecord) => void;
}) {
  if (config.id === 'modules')
    return (
      <Button type="button" onClick={() => onAction('moduleEditor')}>
        <Wrench size={16} aria-hidden />
        Create Module
      </Button>
    );
  if (config.id === 'support-tickets')
    return (
      <>
        <Button type="button" onClick={() => onAction('ticketEditor')}>
          <LifeBuoy size={16} aria-hidden />
          Create Ticket
        </Button>
        <Button type="button" variant="secondary" onClick={() => onAction('reportExport')}>
  ...
  if (config.id === 'knowledge-base')
    return (
      <Button
        type="button"
        onClick={() => onAction(activeTab === 'categories' ? 'categoryEditor' : 'articleEditor')}
      >
        <BookOpen size={16} aria-hidden />
        {activeTab === 'categories' ? 'Create Category' : 'Create Article'}
      </Button>
    );
  if (config.id === 'monitoring')
    return (
      <Button type="button" onClick={() => onAction('incidentEditor')}>
  ...
  if (config.id === 'integrations') {
    if (activeTab === 'providers')
      return (
        <Button type="button" onClick={() => onAction('providerEditor')}>
    ...
    if (activeTab === 'tenant')
      return (
        <Button type="button" onClick={() => onAction('connectProvider')}>
          <PlugZap size={16} aria-hidden />
          Connect Integration
        </Button>
      );
  ...
  if (config.id === 'legal')
    return (
      <Button type="button" onClick={() => onAction('legalEditor')}>
        <FileText size={16} aria-hidden />
        New Document
      </Button>
    );
  if (config.id === 'announcements')
    return (
      <Button type="button" onClick={() => onAction('announcementEditor')}>
        <FileText size={16} aria-hidden />
        New Announcement
      </Button>
    );
  if (config.id === 'webhooks')
    return (
      <Button type="button" onClick={() => onAction('webhookEditor')}>
  ...
  return (
    <Button
      type="button"
      variant="secondary"
      onClick={() => onAction(config.id === 'audit' ? 'auditExport' : 'reportExport')}
    >
      <Download size={16} aria-hidden />
      Export
    </Button>
  ...
  return [
    ...keys.map((key) => ({
      ...
      cell: (row: PlatformRecord) => {
        ...
        if (key.includes('status') || key === 'severity' || key === 'priority')
          return (
            <StatusBadge tone={tone(String(value ?? 'neutral'))}>{printable(value)}</StatusBadge>
          );
      ...
      <PortalActionMenu open={open} anchorRef={triggerRef} onClose={() => setOpen(false)}>
        <div className="action-menu" role="menu">
          <button
            type="button"
            role="menuitem"
            onClick={() => run({ key: 'payload', record: row, tab })}
          >
            <Eye size={15} aria-hidden /> View
          </button>
          {visibleActions.map((entry) => (
            <button
              ...
              type="button"
              role="menuitem"
              onClick={() => run({ key: entry.key, record: row, tab })}
            >
              {entry.icon}
...
function actionLabel(entry: { key: ActionKey; label: string }, row: PlatformRecord) {
  ...
  if (entry.key === 'articlePublish')
    return String(row.status) === 'published' ? 'Unpublish' : 'Publish';
  return entry.label;
}
...
function isActionVisible(key: ActionKey, row: PlatformRecord) {
  ...
  if (key === 'remoteEnd')
    return !['ended', 'expired'].includes(String(row.status ?? '').toLowerCase());
  return true;
}
...
function ActionSurface({
  ...
  const webhookLogRetryMutation = useMutation({
    mutationFn: (logId: string | number) =>
      platformOperationsApi.integrations.retryWebhookLog(logId),
    onSuccess: async () => {
      await queryClient.invalidateQueries({ queryKey: platformQueryKeys.all });
    }
  });
  ...
  const tenantOptionsQuery = useQuery({
    ...
    queryFn: () =>
      safeReferenceList(() => platformOperationsApi.references.tenants({ per_page: 200 })),
    enabled: [
      'ticketEditor',
      'connectProvider',
      'integrationEditor',
  ...
  const planOptionsQuery = useQuery({
    queryKey: platformQueryKeys.list('plan-options', { per_page: 200 }),
    queryFn: () => safeReferenceList(() => platformSubscriptionsApi.plans.list({ per_page: 200 })),
    enabled: action.key === 'trialConvert'
  });
  const platformUserOptionsQuery = useQuery({
    queryKey: platformQueryKeys.list('platform-user-options', { per_page: 200 }),
    queryFn: () =>
      safeReferenceList(() => platformOperationsApi.references.platformUsers({ per_page: 200 })),
  ...
  const providerOptionsQuery = useQuery({
    ...
    queryFn: () =>
      safeReferenceList(() => platformOperationsApi.integrations.providers({ per_page: 200 })),
  ...
  const tenantIntegrationOptionsQuery = useQuery({
    ...
    queryFn: () =>
      safeReferenceList(() =>
        platformOperationsApi.integrations.tenantIntegrations({ per_page: 200 })
      ),
  ...
  const queueJobDetailQuery = useQuery({
    ...
    enabled:
      area === 'monitoring' && action.key === 'queueJobDetail' && Boolean(idOf(action.record))
  });
  const incidentDetailQuery = useQuery({
    ...
    enabled:
      area === 'monitoring' && action.key === 'incidentDetail' && Boolean(idOf(action.record))
  });
  const serviceLogsQuery = useQuery({
    queryKey: platformQueryKeys.related('monitoring-services', idOf(action.record), 'logs'),
    queryFn: () =>
      platformOperationsApi.monitoring.serviceLogs(
        String(action.record?.code ?? idOf(action.record)),
        { per_page: 100 }
      ),
    enabled:
      area === 'monitoring' &&
      action.key === 'serviceLogs' &&
      Boolean(action.record?.code ?? idOf(action.record))
  });
  ...
  const announcementDetailQuery = useQuery({
    ...
    enabled:
      area === 'announcements' &&
      action.key === 'announcementEditor' &&
      Boolean(idOf(action.record))
  });
  ...
  const articleDetailQuery = useQuery({
    ...
    enabled:
      area === 'knowledge-base' &&
      (action.key === 'payload' || action.key === 'articleEditor') &&
      Boolean(idOf(action.record))
  });
  const ticketDetailQuery = useQuery({
    ...
    enabled:
      ...
      (action.key === 'payload' || action.key === 'ticketEditor') &&
      Boolean(idOf(action.record))
  });
  ...
  const tenantIntegrationDetailQuery = useQuery({
    ...
    enabled:
      area === 'integrations' &&
      action.tab === 'tenant' &&
      ['payload', 'integrationEditor'].includes(String(action.key)) &&
      Boolean(idOf(action.record))
  });
  const integrationWebhookDetailQuery = useQuery({
    queryKey: platformQueryKeys.detail('integration-webhooks', idOf(action.record)),
    queryFn: () =>
      platformOperationsApi.integrations.webhook(action.record?.id ?? idOf(action.record)),
    enabled:
      area === 'integrations' &&
      action.tab === 'webhooks' &&
      ['payload', 'integrationWebhookEditor'].includes(String(action.key)) &&
      Boolean(idOf(action.record))
  });
  ...
  const integrationWebhookLogsQuery = useQuery({
    ...
    queryFn: () =>
      platformOperationsApi.integrations.webhookLogs(action.record?.id ?? idOf(action.record)),
    enabled:
      area === 'integrations' &&
      action.key === 'integrationWebhookLogs' &&
      Boolean(idOf(action.record))
  });
  const currentModuleFeaturesQuery = useQuery({
    ...
    enabled:
      area === 'modules' &&
      (action.key === 'payload' || action.key === 'featureAttach') &&
      Boolean(idOf(action.record))
  });
  ...
  if (action.key === 'queueJobDetail') {
    return (
      <AppDrawer
        open
        onClose={onClose}
        title="Queue Job Details"
        guard="platform"
        permission="monitoring.view"
        size="lg"
      >
        {queueJobDetailQuery.isLoading ? (
          <div className="surface-state">Loading queue job...</div>
        ) : null}
        {queueJobDetailQuery.isError ? (
          <div className="surface-error">{errorMessage(queueJobDetailQuery.error)}</div>
        ) : null}
        {!queueJobDetailQuery.isLoading && !queueJobDetailQuery.isError ? (
          <DetailGrid record={queueJobDetailQuery.data} />
        ) : null}
      </AppDrawer>
    );
  }
    ...
    return (
      ...
      >
        {incidentDetailQuery.isLoading ? (
          <div className="surface-state">Loading incident...</div>
        ) : null}
        {incidentDetailQuery.isError ? (
          <div className="surface-error">{errorMessage(incidentDetailQuery.error)}</div>
        ) : null}
        {!incidentDetailQuery.isLoading && !incidentDetailQuery.isError ? (
          <DetailGrid record={incidentDetailQuery.data} />
        ) : null}
      </AppDrawer>
    );
  }
    ...
    return (
      ...
      >
        {serviceLogsQuery.isLoading ? (
          <div className="surface-state">Loading service logs...</div>
        ) : null}
        {serviceLogsQuery.isError ? (
          <div className="surface-error">{errorMessage(serviceLogsQuery.error)}</div>
        ) : null}
        {!serviceLogsQuery.isLoading && !serviceLogsQuery.isError ? (
          <RelatedRows title="Service Logs" rows={serviceLogsQuery.data?.data ?? []} />
        ) : null}
      </AppDrawer>
    );
  }
  if (action.key === 'payload' || action.key === 'auditCompare') {
    return (
      <AppDrawer
        open
        onClose={onClose}
        title={detailTitleFor(area, action)}
        guard="platform"
        permission={viewPermissionFor(area, action.key)}
        size="lg"
      >
        <DetailGrid
          record={mask(
            detailRecordFor(area, action, {
        ...
        {area === 'modules' ? (
          <>
            <RelatedRows
              title="Features"
              rows={arrayFromResponse(currentModuleFeaturesQuery.data?.data, 'features')}
            />
        ...
        {area === 'support-tickets' ? (
          <>
            <RelatedRows
              title="Comments"
              rows={arrayFromResponse(ticketDetailQuery.data, 'comments')}
            />
            <RelatedRows
              title="Attachments"
              rows={arrayFromResponse(ticketDetailQuery.data, 'attachments')}
            />
  ...
  if (action.key === 'integrationRateLimits' || action.key === 'integrationWebhookLogs') {
    const rows = arrayFromResponse(
      action.key === 'integrationRateLimits'
        ? integrationRateLimitsQuery.data?.data
        : integrationWebhookLogsQuery.data?.data,
      ...
      <AppDrawer
        ...
        footer={
          <>
            ...
            <Button
              type="button"
              onClick={() => mutation.mutate(normalizePayload(action.key, payload))}
      ...
      >
        <GenericFields
          ...
          references={referenceOptions({
            ...
            plans: planOptionsQuery.data?.data
          })}
          onChange={setPayload}
        />
      </AppDrawer>
        ...
        <label key={field.name} className={field.type === 'checkbox' ? 'check-row' : undefined}>
          {field.type === 'checkbox' ? (
            <>
              <input
                type="checkbox"
                checked={Boolean(payload[field.name])}
                onChange={(event) => onChange({ ...payload, [field.name]: event.target.checked })}
              />
              <FieldLabel required={field.required}>{field.label}</FieldLabel>
            </>
          ) : (
            <>
              ...
              {field.type === 'textarea' ? (
                <textarea
                  ...
                  onChange={(event) => onChange({ ...payload, [field.name]: event.target.value })}
                />
                ...
                >
                  ...
                  {references.categories.map((category) => (
                    <option key={idOf(category)} value={idOf(category)}>
                      {printable(category.name ?? category.slug)}
                    </option>
                  ))}
                </select>
              ) : field.reference ? (
                <ReferenceSelect
                  ...
                  value={String(payload[field.name] ?? '')}
                  onChange={(value) => onChange({ ...payload, [field.name]: value })}
                />
              ) : field.type === 'select' ? (
                <select
                  required={field.required}
                  value={String(payload[field.name] ?? field.options?.[0] ?? '')}
                  onChange={(event) => onChange({ ...payload, [field.name]: event.target.value })}
                >
                  {field.options?.map((option) => (
                    <option key={option} value={option}>
                ...
                <input
                  ...
                  type="file"
                  onChange={(event) =>
                    onChange({ ...payload, [field.name]: event.target.files?.[0] ?? null })
                  }
                />
              ) : (
                <input
...
function ReferenceSelect({
  ...
  const optionValue = (option: PlatformRecord) =>
    reference === 'providers' ? String(option.code ?? idOf(option)) : idOf(option);
...
function referenceRows(reference: NonNullable<Field['reference']>, references: ReferenceOptions) {
  ...
  if (reference === 'plans') return references.plans;
  return references.features;
}

function referenceLabel(reference: NonNullable<Field['reference']>, record: PlatformRecord) {
  if (reference === 'tenants')
    return printable(record.organization_name ?? record.name ?? record.slug ?? record.uuid);
  if (reference === 'platformUsers')
    return printable(record.display_name ?? record.name ?? record.email ?? record.uuid);
  if (reference === 'files')
    return printable(record.original_name ?? record.name ?? record.filename ?? record.uuid);
  ...
  if (reference === 'tenantIntegrations')
    return printable(record.name ?? record.provider_code ?? record.uuid);
  if (reference === 'plans')
    return printable(record.name ?? record.display_name ?? record.code ?? record.uuid);
      ...
      {options.map((option) => {
        ...
        return (
          <label key={value} className="check-row">
            ...
            />
            <span>
              {printable(option.name ?? option.code)} / {printable(option.module)}
            </span>
          </label>
        );
      })}
...
async function runAction(
  ...
  if (area === 'modules') {
    if (action.key === 'moduleEditor')
      return id
        ? platformOperationsApi.modules.update(id, payload)
        : platformOperationsApi.modules.create(payload);
    if (action.key === 'moduleToggle')
      return String(action.record?.status) === 'active'
        ? platformOperationsApi.modules.disable(id, payload)
        : platformOperationsApi.modules.enable(id, payload);
    if (action.key === 'featureAttach')
      return platformOperationsApi.modules.replaceFeatures(id, values(payload.feature_uuids));
  }
  if (area === 'support-tickets') {
    if (action.key === 'ticketEditor')
      return id
        ? platformOperationsApi.support.tickets.update(id, payload)
        : platformOperationsApi.support.tickets.create(payload);
    if (action.key === 'ticketAssign')
      return platformOperationsApi.support.tickets.assign(id, payload);
    if (action.key === 'ticketReply')
      return platformOperationsApi.support.tickets.comment(id, payload);
    ...
    if (action.key === 'ticketClose')
      return platformOperationsApi.support.tickets.close(id, payload);
    if (action.key === 'ticketReopen')
      return platformOperationsApi.support.tickets.reopen(id, payload);
    if (action.key === 'reportExport')
      return platformOperationsApi.support.tickets.export({ selected_ids: selectedIds });
  }
  if (area === 'knowledge-base') {
    if (action.key === 'articleEditor')
      return id
        ? platformOperationsApi.support.articles.update(id, payload)
        : platformOperationsApi.support.articles.create(payload);
    if (action.key === 'articlePublish')
      return String(action.record?.status) === 'published'
        ? platformOperationsApi.support.articles.unpublish(id, payload)
        : platformOperationsApi.support.articles.publish(id, payload);
    if (action.key === 'articleArchive')
      return platformOperationsApi.support.articles.archive(id, payload);
    if (action.key === 'categoryEditor')
      return id
        ? platformOperationsApi.support.kbCategories.update(id, payload)
        : platformOperationsApi.support.kbCategories.create(payload);
  }
  if (area === 'remote-login' && action.key === 'remoteEnd')
    return platformOperationsApi.support.remoteSessions.end(id, payload);
  if (area === 'reports' && action.key === 'reportExport')
    return platformOperationsApi.reports.export(String(action.tab ?? 'revenue'), payload);
  if (area === 'monitoring') {
    if (action.key === 'alertResolve')
      return platformOperationsApi.monitoring.resolveAlert(action.record?.id ?? id, payload);
    if (action.key === 'incidentEditor')
      return id
        ? platformOperationsApi.monitoring.updateIncident(action.record?.id ?? id, payload)
        : platformOperationsApi.monitoring.createIncident(payload);
    if (action.key === 'retry' && action.tab === 'queue')
      return platformOperationsApi.monitoring.retryQueueJob(action.record?.id ?? id, payload);
    if (action.key === 'queueDelete' && action.tab === 'queue')
      return platformOperationsApi.monitoring.deleteQueueJob(action.record?.id ?? id, payload);
    if (action.key === 'incidentResolve')
      return platformOperationsApi.monitoring.resolveIncident(action.record?.id ?? id, payload);
  }
  if (area === 'integrations') {
    if (action.key === 'providerEditor')
      return id
        ? platformOperationsApi.integrations.updateProvider(id, payload)
        : platformOperationsApi.integrations.createProvider(payload);
    if (action.key === 'connectProvider')
      return platformOperationsApi.integrations.createTenantIntegration(payload);
    if (action.key === 'integrationEditor')
      return platformOperationsApi.integrations.updateTenantIntegration(id, payload);
    if (action.key === 'integrationTest') return platformOperationsApi.integrations.test(id);
    if (action.key === 'integrationDisconnect')
      return platformOperationsApi.integrations.disconnect(id, payload);
    if (action.key === 'rotateCredentials')
      return platformOperationsApi.integrations.rotateCredentials(id, {
        credentials: credentialsFromPayload(payload)
      });
    if (action.key === 'fieldMapping')
      return platformOperationsApi.integrations.replaceMappings(
        id,
        fieldMappingsFromPayload(payload)
      );
    if (action.key === 'integrationWebhookEditor')
      return id
        ? platformOperationsApi.integrations.updateWebhook(action.record?.id ?? id, payload)
        : platformOperationsApi.integrations.createWebhook(payload);
    if (action.key === 'integrationWebhookDisable')
      return platformOperationsApi.integrations.disableWebhook(action.record?.id ?? id);
    if (action.key === 'retry' && action.tab === 'sync')
      return platformOperationsApi.integrations.retrySyncJob(action.record?.id ?? id, payload);
    if (action.key === 'retry')
      return platformOperationsApi.integrations.retryWebhookLog(action.record?.id ?? id, payload);
  }
  if (area === 'settings') {
    if (action.key === 'settingsEditor')
      return platformOperationsApi.settings.updatePlatform({ settings: json(payload.settings) });
    if (action.key === 'backupRun') return platformOperationsApi.settings.runBackup(payload);
    if (action.key === 'templateEditor')
      return platformOperationsApi.settings.createTemplate(normalizePayload(action.key, payload));
  }
  if (area === 'audit') {
    if (action.key === 'securityReview')
      return platformOperationsApi.audit.reviewSecurity(id, {
        review_status: payload.review_status,
        review_notes: payload.review_notes
      });
    if (action.key === 'announcementArchive' || action.key === 'announcementDelete')
      return { reason: '' };
    if (action.key === 'announcementEditor')
      return {
    ...
    if (action.key === 'auditExport')
      return platformOperationsApi.audit.export({ ...payload, selected_ids: selectedIds });
  }
  if (area === 'trials') {
    if (action.key === 'trialExtend')
      return platformOperationsApi.lifecycle.extendTrial(id, payload);
    if (action.key === 'trialConvert')
      return platformOperationsApi.lifecycle.convertTrial(id, payload);
  }
  if (area === 'legal') {
    if (action.key === 'legalEditor')
      return id
        ? platformOperationsApi.lifecycle.updateLegal(id, payload)
        : platformOperationsApi.lifecycle.createLegal(payload);
    if (action.key === 'legalPublish') return platformOperationsApi.lifecycle.publishLegal(id);
  }
  if (area === 'announcements') {
    if (action.key === 'announcementEditor')
      return id
        ? platformOperationsApi.lifecycle.updateAnnouncement(id, payload)
        : platformOperationsApi.lifecycle.createAnnouncement(payload);
    if (action.key === 'announcementPublish')
      return platformOperationsApi.lifecycle.publishAnnouncement(id);
    if (action.key === 'announcementArchive')
      return platformOperationsApi.lifecycle.archiveAnnouncement(id, payload);
    if (action.key === 'announcementDelete')
      return platformOperationsApi.lifecycle.deleteAnnouncement(id, payload);
  }
  if (area === 'webhooks') {
    if (action.key === 'webhookEditor')
      return id
        ? platformOperationsApi.webhooks.updateEndpoint(id, normalizePayload(action.key, payload))
        : platformOperationsApi.webhooks.createEndpoint(normalizePayload(action.key, payload));
...
const configs: Record<OperationArea, AreaConfig> = {
  modules: {
    ...
    description:
      'Global modules, feature attachments, tenant overrides, and implementation placeholders for missing persistence.',
    ...
    tabs: [
      {
        ...
        label: 'Modules',
        resource: 'modules',
        query: platformOperationsApi.modules.list,
        columns: ['name', 'code', 'category', 'is_core', 'status', 'sort_order'],
        primary: ['name'],
        actions: [
          { key: 'moduleEditor', label: 'Edit', icon: <Pencil size={14} aria-hidden /> },
          { key: 'moduleToggle', label: 'Toggle', icon: <CheckCircle2 size={14} aria-hidden /> },
          { key: 'featureAttach', label: 'Features', icon: <Wrench size={14} aria-hidden /> }
        ]
      }
    ]
  },
  'support-tickets': {
    ...
    description:
      'Ticket queue, assignment, replies, internal notes, close/reopen flow, export, and linked payload views.',
    ...
    tabs: [
      {
        ...
        query: platformOperationsApi.support.tickets.list,
        columns: [
          'ticket_number',
          'tenant_name',
          'subject',
        ...
        primary: ['ticket_number', 'subject'],
        actions: [
          { key: 'ticketEditor', label: 'Edit', icon: <Pencil size={14} aria-hidden /> },
          { key: 'ticketAssign', label: 'Assign', icon: <UserCheck size={14} aria-hidden /> },
          {
            key: 'ticketReply',
            label: 'Reply',
            icon: <MessageSquareReply size={14} aria-hidden />
          },
          { key: 'ticketAttach', label: 'Attach File', icon: <Paperclip size={14} aria-hidden /> },
          { key: 'ticketClose', label: 'Close', icon: <CheckCircle2 size={14} aria-hidden /> },
          { key: 'ticketReopen', label: 'Reopen', icon: <RefreshCw size={14} aria-hidden /> }
        ]
      }
    ]
  },
  'knowledge-base': {
    ...
    description:
      'Categories and articles with editor drawer plus publish, unpublish, archive ready actions.',
    ...
    tabs: [
      {
        ...
        query: platformOperationsApi.support.articles.list,
        columns: ['title', 'slug', 'audience', 'status', 'published_at', 'updated_at'],
        primary: ['title'],
        actions: [
          { key: 'articleEditor', label: 'Edit', icon: <Pencil size={14} aria-hidden /> },
          { key: 'articlePublish', label: 'Publish', icon: <Send size={14} aria-hidden /> },
          { key: 'articleArchive', label: 'Archive', icon: <Archive size={14} aria-hidden /> }
        ]
      },
      {
        id: 'categories',
        ...
        query: platformOperationsApi.support.kbCategories.list,
        columns: ['name', 'slug', 'audience', 'status', 'updated_at'],
        primary: ['name'],
        actions: [{ key: 'categoryEditor', label: 'Edit', icon: <Pencil size={14} aria-hidden /> }]
      }
    ]
  },
  'remote-login': {
    ...
    tabs: [
      {
        id: 'sessions',
        label: 'Sessions',
        resource: 'remote-login-sessions',
        query: platformOperationsApi.support.remoteSessions.list,
        columns: [
          'tenant_name',
          'platform_user_name',
          'target_user_name',
        ...
        primary: ['reason'],
        actions: [{ key: 'remoteEnd', label: 'End', icon: <Ban size={14} aria-hidden /> }]
      }
    ]
  },
  reports: {
    ...
    title: 'Reports',
    description:
      'Operational report pages with filters, drill-down rows, export queue integration, and export job tracking.',
  ...
  monitoring: {
    ...
    title: 'Monitoring',
    description:
      'Service health, jobs, scheduler logs, API logs, alerts, incidents, and raw exception drawers.',
    permission: 'monitoring.view',
    tabs: [
      {
        id: 'services',
        ...
        query: platformOperationsApi.monitoring.services,
        columns: ['name', 'code', 'service_type', 'status', 'check_interval_seconds'],
        primary: ['name'],
        actions: [
          { key: 'serviceLogs', label: 'View Logs', icon: <FileJson2 size={14} aria-hidden /> }
        ]
      },
      {
        id: 'queue',
        label: 'Queue Jobs',
        resource: 'queue-jobs',
        query: platformOperationsApi.monitoring.queueJobs,
        columns: ['id', 'queue', 'job_name', 'status', 'attempts', 'started_at', 'finished_at'],
        actions: [
          {
            key: 'queueJobDetail',
            label: 'View Details',
            icon: <FileJson2 size={14} aria-hidden />
          },
          { key: 'retry', label: 'Retry', icon: <RotateCw size={14} aria-hidden /> },
          { key: 'queueDelete', label: 'Delete', icon: <Trash2 size={14} aria-hidden /> },
          { key: 'payload', label: 'Exception', icon: <FileJson2 size={14} aria-hidden /> }
        ]
      },
      {
        id: 'scheduler',
        ...
        query: platformOperationsApi.monitoring.schedulerLogs,
        columns: ['command', 'status', 'duration_ms', 'started_at', 'finished_at']
      },
      {
        id: 'api',
        label: 'API Logs',
        resource: 'api-request-logs',
        query: platformOperationsApi.monitoring.apiLogs,
        columns: [
          'tenant_name',
          'user_name',
          'method',
      ...
      },
      {
        ...
        query: platformOperationsApi.monitoring.alerts,
        columns: ['id', 'severity', 'message', 'status', 'triggered_at', 'resolved_at'],
        primary: ['message'],
        actions: [
          { key: 'alertResolve', label: 'Resolve', icon: <CheckCircle2 size={14} aria-hidden /> }
        ]
      },
      {
        id: 'incidents',
        label: 'Incidents',
        resource: 'system-incidents',
        query: platformOperationsApi.monitoring.incidents,
        ...
        actions: [
          ...
          { key: 'incidentEditor', label: 'Edit', icon: <Pencil size={14} aria-hidden /> },
          { key: 'incidentResolve', label: 'Resolve', icon: <CheckCircle2 size={14} aria-hidden /> }
        ]
      },
      {
        id: 'usage',
        label: 'Usage Snapshots',
        resource: 'tenant-usage-snapshots',
        query: platformOperationsApi.monitoring.usage,
        columns: [
          'tenant_name',
          'period_start',
          'period_end',
  ...
  integrations: {
    ...
    title: 'Integrations',
    description:
      'Provider catalog, tenant integrations, credentials, sync jobs, mappings, and rate-limit surfaces.',
    permission: 'integration.view',
    tabs: [
      {
        id: 'providers',
        ...
        query: platformOperationsApi.integrations.providers,
        columns: ['name', 'code', 'category', 'auth_type', 'status'],
        primary: ['name'],
        actions: [{ key: 'providerEditor', label: 'Edit', icon: <Pencil size={14} aria-hidden /> }]
      },
      {
        id: 'tenant',
        label: 'Tenant Integrations',
        resource: 'tenant-integrations',
        query: platformOperationsApi.integrations.tenantIntegrations,
        columns: ['uuid', 'tenant_name', 'provider_code', 'name', 'status', 'connected_at'],
        primary: ['name'],
        actions: [
          { key: 'integrationEditor', label: 'Edit', icon: <Pencil size={14} aria-hidden /> },
          { key: 'integrationTest', label: 'Test', icon: <CheckCircle2 size={14} aria-hidden /> },
          {
            key: 'rotateCredentials',
            label: 'Rotate',
            icon: <LockKeyhole size={14} aria-hidden />
          },
          { key: 'fieldMapping', label: 'Mappings', icon: <Split size={14} aria-hidden /> },
          {
            key: 'integrationRateLimits',
            label: 'Rate Limits',
            icon: <Clock size={14} aria-hidden />
          },
          { key: 'integrationDisconnect', label: 'Disconnect', icon: <Ban size={14} aria-hidden /> }
        ]
      },
      {
        id: 'sync',
        label: 'Sync Jobs',
        resource: 'integration-sync-jobs',
        query: platformOperationsApi.integrations.syncJobs,
        columns: [
          'tenant_name',
          'integration_name',
          'sync_type',
        ...
        actions: [
          { key: 'retry', label: 'Retry', icon: <RotateCw size={14} aria-hidden /> },
          { key: 'payload', label: 'Payload', icon: <FileJson2 size={14} aria-hidden /> }
        ]
      }
    ]
  },
  settings: {
    ...
    description:
      'Grouped platform settings, notification templates, backups, and backup run history.',
    ...
    tabs: [
      {
        ...
        query: async () => settingsList(),
        columns: ['group', 'key', 'value_type', 'updated_at'],
        primary: ['group', 'key'],
        actions: [
          { key: 'settingsEditor', label: 'Edit', icon: <Settings2 size={14} aria-hidden /> }
        ]
      },
      {
        id: 'templates',
        ...
        query: platformOperationsApi.settings.templates,
        columns: ['code', 'channel', 'subject', 'status', 'updated_at'],
        primary: ['code'],
        actions: [{ key: 'templateEditor', label: 'Edit', icon: <Pencil size={14} aria-hidden /> }]
      },
      {
        id: 'backups',
        label: 'Backups',
        resource: 'backup-runs',
        query: platformOperationsApi.settings.backupRuns,
        columns: ['uuid', 'backup_type', 'status', 'size_bytes', 'started_at', 'finished_at'],
        actions: [{ key: 'backupRun', label: 'Run', icon: <UploadCloud size={14} aria-hidden /> }]
      }
    ]
  },
  audit: {
    ...
    title: 'Audit Logs',
    description:
      'Activity, security, billing/payment/subscription/system and remote-login audit surfaces with compare and export.',
    ...
    tabs: [
      {
        ...
        query: platformOperationsApi.audit.activity,
        columns: [
          'id',
          'actor_name',
          'tenant_name',
        ...
        primary: ['event'],
        actions: [
          { key: 'auditCompare', label: 'Compare', icon: <CopyCheck size={14} aria-hidden /> }
        ]
      },
      {
        id: 'security',
        ...
        query: platformOperationsApi.audit.security,
        columns: [
          'id',
          'tenant_name',
          'user_name',
        ...
        primary: ['event'],
        actions: [
          { key: 'securityReview', label: 'Review', icon: <ClipboardCheck size={14} aria-hidden /> }
        ]
      }
    ]
  },
  onboarding: {
    ...
    title: 'Onboarding',
    description:
      'Tenant onboarding queue with progress rows and implementation-ready step update flow.',
    permission: 'tenant.view',
    tabs: [
      {
        id: 'tenants',
        label: 'Tenant Queue',
        resource: 'onboarding-tenants',
        query: platformOperationsApi.lifecycle.onboarding,
        columns: ['uuid', 'organization_name', 'status', 'steps'],
    ...
    tabs: [
      {
        ...
        label: 'Trials',
        resource: 'trials',
        query: platformOperationsApi.lifecycle.trials,
        columns: ['uuid', 'organization_name', 'status', 'trial_ends_at', 'created_at'],
        primary: ['organization_name'],
        actions: [
          { key: 'trialExtend', label: 'Extend', icon: <Clock size={14} aria-hidden /> },
          { key: 'trialConvert', label: 'Convert', icon: <Repeat2 size={14} aria-hidden /> }
        ]
      }
    ]
  },
  legal: {
    ...
    description:
      'Terms, privacy, DPA, tenant agreement versions, publishing, and acceptance review shell.',
    ...
    tabs: [
      {
        ...
        query: platformOperationsApi.lifecycle.legal,
        columns: ['document_type', 'title', 'version', 'status', 'published_at', 'updated_at'],
        primary: ['title'],
        actions: [
          { key: 'legalEditor', label: 'Edit', icon: <Pencil size={14} aria-hidden /> },
          { key: 'legalPublish', label: 'Publish', icon: <Send size={14} aria-hidden /> }
        ]
      }
    ]
  },
    ...
    tabs: [
      {
        ...
        query: platformOperationsApi.lifecycle.announcements,
        columns: ['title', 'audience', 'status', 'published_at', 'created_at'],
        primary: ['title'],
        actions: [
          { key: 'announcementEditor', label: 'Edit', icon: <Pencil size={14} aria-hidden /> },
          { key: 'announcementPublish', label: 'Publish', icon: <Send size={14} aria-hidden /> },
          { key: 'announcementArchive', label: 'Archive', icon: <Archive size={14} aria-hidden /> },
          { key: 'announcementDelete', label: 'Delete', icon: <Trash2 size={14} aria-hidden /> }
        ]
      }
    ]
  },
    ...
    tabs: [
      {
        ...
        query: platformOperationsApi.webhooks.endpoints,
        columns: ['uuid', 'name', 'url', 'status', 'created_at'],
        primary: ['name'],
        actions: [{ key: 'webhookEditor', label: 'Edit', icon: <Link2 size={14} aria-hidden /> }]
      }
    ]
  }
};
...
function reportTabs(): AreaConfig['tabs'] {
  ...
  return [
    ...
    })),
    {
      id: 'export-jobs',
      label: 'Export Jobs',
      resource: 'report-export-jobs',
      query: platformOperationsApi.reports.jobs,
      columns: ['uuid', 'report_code', 'format', 'status', 'created_at', 'finished_at'],
      primary: ['report_code']
    }
  ];
}
...
function fieldsFor(area: OperationArea, action: ActionState): Field[] {
  ...
  if (action.key === 'ticketEditor')
    return [
      { name: 'tenant_uuid', label: 'Tenant', reference: 'tenants' },
      ...
      },
      {
        ...
        label: 'Category',
        type: 'select',
        options: ['general', 'billing', 'technical', 'account', 'feature_request']
      },
      {
        name: 'source',
        label: 'Source',
        type: 'select',
        options: ['platform', 'tenant_help_center', 'email', 'phone', 'chat']
      },
      { name: 'assigned_to_uuid', label: 'Assignee', reference: 'platformUsers' },
      {
        name: 'status',
        label: 'Status',
        type: 'select',
  ...
  if (action.key === 'ticketAssign')
    return [
      { name: 'assigned_to_uuid', label: 'Assignee', reference: 'platformUsers' },
      { name: 'audit_reason', label: 'Audit Reason', type: 'textarea' }
    ];
  if (action.key === 'ticketReply')
    return [
      {
        name: 'comment',
        label: area === 'audit' ? 'Review Notes' : 'Reply / Internal Note',
        type: 'textarea',
        required: true
      },
      { name: 'is_internal', label: 'Internal note', type: 'checkbox' }
    ];
  if (action.key === 'ticketAttach')
    return [{ name: 'upload_file', label: 'File', type: 'file', required: true }];
  if (action.key === 'ticketClose')
    return [{ name: 'notes', label: 'Resolution Notes', type: 'textarea' }];
  if (action.key === 'ticketReopen') return [];
  if (action.key === 'articleEditor')
    return [
      { name: 'category_uuid', label: 'Category', reference: 'categories' },
      { name: 'title', label: 'Title', required: true },
      {
        name: 'audience',
        label: 'Audience',
        type: 'select',
        options: ['all', 'public', 'tenant', 'internal']
      },
      {
        name: 'status',
        label: 'Status',
  ...
  if (action.key === 'categoryEditor')
    return [
      { name: 'parent_uuid', label: 'Parent Category', reference: 'categories' },
      { name: 'name', label: 'Name', required: true },
      {
        name: 'audience',
        label: 'Audience',
  ...
  if (action.key === 'reportExport')
    return [
      { name: 'format', label: 'Format', type: 'select', options: ['csv', 'xlsx', 'pdf'] },
      { name: 'delivery', label: 'Delivery', type: 'select', options: ['job', 'download'] },
      { name: 'email_when_ready', label: 'Email when ready', type: 'checkbox' }
    ];
  if (action.key === 'alertResolve')
    return [{ name: 'resolution_notes', label: 'Resolution Notes', type: 'textarea' }];
  if (action.key === 'incidentResolve')
    return [{ name: 'resolution_notes', label: 'Resolution Notes', type: 'textarea' }];
  if (action.key === 'incidentEditor')
    return [
  ...
  if (action.key === 'connectProvider' || action.key === 'integrationEditor')
    return [
      { name: 'tenant_uuid', label: 'Tenant', reference: 'tenants', required: true },
      { name: 'provider_code', label: 'Provider', reference: 'providers', required: true },
      { name: 'name', label: 'Connection Name', required: true },
      {
        name: 'status',
        label: 'Status',
        type: 'select',
        options: ['active', 'inactive', 'disconnected']
      },
      { name: 'credential_api_key', label: 'API Key' },
      { name: 'credential_secret', label: 'Secret' },
      { name: 'credential_token', label: 'Access Token' }
    ];
  if (action.key === 'rotateCredentials')
    return [
      { name: 'credential_api_key', label: 'API Key' },
    ...
    return [
      {
        ...
        type: 'select',
        required: true,
        options: ['customer', 'contact', 'invoice', 'payment', 'ticket', 'lead', 'deal']
      },
      { name: 'local_field', label: 'Local Field', required: true },
      { name: 'external_field', label: 'External Field', required: true },
      {
        name: 'transform_rule',
        label: 'Transform Rule',
        type: 'select',
        options: ['none', 'uppercase', 'lowercase', 'trim', 'date_iso']
      }
    ];
  if (action.key === 'integrationWebhookEditor')
    return [
  ...
  if (action.key === 'integrationDisconnect')
    return [{ name: 'reason', label: 'Reason', type: 'textarea' }];
  if (action.key === 'settingsEditor')
    return [{ name: 'settings', label: 'Settings JSON by Group', type: 'textarea' }];
  if (action.key === 'backupRun')
    return [
      { name: 'backup_type', label: 'Backup Type', type: 'select', options: ['full', 'database'] }
    ];
  if (action.key === 'templateEditor')
    return [
      ...
      { name: 'variables', label: 'Variables CSV' },
      { name: 'status', label: 'Status', type: 'select', options: ['active', 'inactive'] }
    ];
  if (action.key === 'securityReview')
    return [
      {
        ...
        label: 'Review Status',
        type: 'select',
        options: ['reviewed', 'dismissed', 'escalated'],
        required: true
      },
      { name: 'review_notes', label: 'Review Notes', type: 'textarea', required: true }
    ];
  if (action.key === 'auditExport')
    return [
      { name: 'format', label: 'Format', type: 'select', options: ['csv'] },
      { name: 'delivery', label: 'Delivery', type: 'select', options: ['job', 'download'] },
      { name: 'scope', label: 'Scope', type: 'select', options: ['filtered', 'selected'] },
      { name: 'email_when_ready', label: 'Email when ready', type: 'checkbox' }
    ];
  if (action.key === 'trialExtend')
    return [
      { name: 'trial_ends_at', label: 'New Trial End', type: 'date' },
      { name: 'reason', label: 'Reason', type: 'textarea' }
    ];
  if (action.key === 'trialConvert')
    return [
      { name: 'plan_uuid', label: 'Plan', reference: 'plans' },
      {
        name: 'billing_cycle',
        label: 'Billing Cycle',
        type: 'select',
    ...
    return [
      {
        ...
        label: 'Type',
        type: 'select',
        options: ['terms_and_conditions', 'privacy_policy', 'dpa', 'tenant_agreement'],
        required: true
      },
      { name: 'title', label: 'Title', required: true },
      { name: 'version', label: 'Version', required: true },
      { name: 'status', label: 'Status', type: 'select', options: ['draft', 'review'] },
  ...
  if (action.key === 'announcementEditor')
    return [
      { name: 'title', label: 'Title', required: true },
      {
        ...
        label: 'Audience',
        type: 'select',
        options: ['all', 'platform_users', 'tenants']
      },
      {
        name: 'status',
        label: 'Status',
        type: 'select',
        options: ['draft', 'review', 'scheduled', 'cancelled']
      },
      { name: 'body', label: 'Body', type: 'textarea', required: true }
    ];
  if (action.key === 'webhookEditor')
...
function normalizePayload(action: ActionKey, payload: Record<string, unknown>) {
  ...
  if (action === 'integrationWebhookEditor' || action === 'webhookEditor')
    return { ...stripEmpty(payload), events: csv(payload.events) };
  if (action === 'connectProvider')
    return stripEmpty({
      ...withoutCredentialFields(payload),
      credentials: credentialsFromPayload(payload)
    });
...
function fieldMappingsFromPayload(payload: Record<string, unknown>) {
  return [
    stripEmpty({
      ...
      transform_rule:
        payload.transform_rule && payload.transform_rule !== 'none'
          ? { rule: payload.transform_rule }
          : undefined
    })
  ];
}
...
function defaultPayloadFor(action: ActionState): Record<string, unknown> {
  ...
  if (action.key === 'ticketAssign')
    return { assigned_to_uuid: action.record?.assigned_to_uuid ?? '', audit_reason: '' };
  ...
  if (action.key === 'articleEditor') {
    return {
      category_uuid:
        action.record?.category_uuid ??
        (action.record?.category as PlatformRecord | undefined)?.uuid ??
        '',
      title: action.record?.title ?? '',
      audience: action.record?.audience ?? 'all',
      status: action.record?.status ?? 'draft',
  ...
  if (action.key === 'categoryEditor') {
    return {
      parent_uuid:
        action.record?.parent_uuid ??
        (action.record?.parent as PlatformRecord | undefined)?.uuid ??
        '',
      name: action.record?.name ?? '',
      audience: action.record?.audience ?? 'all',
      status: action.record?.status ?? 'active'
  ...
  if (action.key === 'rotateCredentials')
    return { credential_api_key: '', credential_secret: '', credential_token: '' };
  if (action.key === 'fieldMapping')
    return { entity_type: 'customer', local_field: '', external_field: '', transform_rule: 'none' };
  if (action.key === 'integrationWebhookEditor') {
    return {
      integration_uuid:
        action.record?.integration_uuid ??
        action.record?.tenant_integration_uuid ??
        action.record?.tenant_integration_id ??
        '',
...
function detailRecordFor(
  ...
  if (area === 'knowledge-base' && action.tab === 'articles')
    return records.article ?? action.record ?? {};
  ...
  if (area === 'integrations' && action.tab === 'tenant')
    return records.tenantIntegration ?? action.record ?? {};
  if (area === 'integrations' && action.tab === 'webhooks')
    return records.integrationWebhook ?? action.record ?? {};
        ...
        <div className="record-list">
          {rows.map((row, index) => (
            <article key={idOf(row) || index}>
              <strong>
                {printable(
                  row.tenant_name ?? row.name ?? row.organization_name ?? row.code ?? row.uuid
                )}
              </strong>
          ...
          {rows.map((row, index) => {
            ...
            return (
              <article key={String(logId || index)}>
                ...
                <p>
                  {printable(
                    row.response_status ?? row.status_code ?? row.message ?? row.error_message
                  )}
                </p>
...
function TicketComments({ rows }: { rows: PlatformRecord[] }) {
  return (
    <section className="dashboard-panel">
      <h3>Comments</h3>
      {rows.length === 0 ? (
        <div className="empty-state">No comments added.</div>
      ) : (
        <div className="record-list">
          {rows.map((row, index) => (
            <article key={idOf(row) || index}>
              <header>
                ...
                <StatusBadge tone={row.is_internal ? 'warning' : 'info'}>
                  {row.is_internal ? 'Internal note' : 'Reply'}
                </StatusBadge>
              </header>
...
function TicketAttachments({ rows }: { rows: PlatformRecord[] }) {
  return (
    <section className="dashboard-panel">
      <h3>Attachments</h3>
      {rows.length === 0 ? (
        <div className="empty-state">No attachments added.</div>
      ) : (
              ...
              <article key={idOf(row) || String(row.file_uuid ?? index)}>
                <header>
                  <div>
                    ...
                    <p>
                      {printable(mime)} / {formatBytes(row.size_bytes)}
                    </p>
                  </div>
                  {url ? (
                ...
                <div className="attachment-preview">
                  {url && mime.startsWith('image/') ? (
                    <img src={url} alt={String(row.original_name ?? 'Attachment preview')} />
                  ) : null}
                  {url && mime === 'application/pdf' ? (
                    <iframe src={url} title={String(row.original_name ?? 'PDF attachment')} />
                  ) : null}
                  {!url || (!mime.startsWith('image/') && mime !== 'application/pdf') ? (
                    <div className="surface-state">Preview is available for images and PDFs.</div>
                  ) : null}
                </div>
                <small>
                  Uploaded by {printable(row.created_by_name ?? 'platform')} on{' '}
                  {printable(row.created_at)}
                </small>
              </article>
            );
          })}
...
function TicketAudit({ rows }: { rows: PlatformRecord[] }) {
  return (
    <section className="dashboard-panel">
      <h3>Audit</h3>
      {rows.length === 0 ? (
        <div className="empty-state">No audit entries returned.</div>
      ) : (
        <div className="record-list">
          {rows.map((row, index) => (
            <article key={idOf(row) || index}>
...
function permissionFor(area: OperationArea, action: ActionKey) {
  if (['moduleEditor', 'moduleToggle', 'featureAttach', 'tenantOverride'].includes(String(action)))
    return 'module.edit';
  if (action === 'articlePublish') return 'support.knowledge_base.publish';
  if (['articleEditor', 'articleArchive', 'categoryEditor'].includes(String(action)))
    return 'support.knowledge_base.edit';
  if (area === 'knowledge-base') return 'support.knowledge_base.view';
  ...
  if (['ticketEditor', 'ticketReply', 'ticketAttach'].includes(String(action)))
  ...
  if (['announcementArchive', 'announcementDelete'].includes(String(action)))
...
function DetailGrid({ record }: { record: unknown }) {
  if (!record || typeof record !== 'object')
    return <div className="empty-state">No details returned.</div>;
  const entries = Object.entries(record as Record<string, unknown>).filter(
    ([key, value]) => value !== null && value !== undefined && value !== '' && !isTechnicalKey(key)
  );
  if (entries.length === 0)
    return <div className="empty-state">No displayable details returned.</div>;
  return (
...
function tone(value: string): 'neutral' | 'success' | 'warning' | 'danger' | 'info' {
  ...
  if (['active', 'published', 'resolved', 'success', 'completed', 'healthy'].includes(lower))
    return 'success';
  if (['pending', 'draft', 'queued', 'investigating', 'warning', 'medium'].includes(lower))
    return 'warning';
  if (['failed', 'critical', 'high', 'inactive', 'closed', 'archived'].includes(lower))
    return 'danger';
  return 'neutral';
}
